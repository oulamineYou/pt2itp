...
function post(feat) {
    ...
    if (feat.geometry.type === 'Point') {
        centre = turf.pointOnSurface(feat);

        tiles = tiles.concat(cover.tiles(feat.geometry, { min_zoom: 14, max_zoom: 14 }));
    } else {
        //Address Cluster Centroid first fallback to network
        centre = turf.pointOnSurface(feat.geometry.geometries[feat.geometry.geometries[1] ? 1 : 0]);
