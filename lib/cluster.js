...
class Cluster {
    ...
    break(segs) {
        ...
        for (let seg of segs) {
            ...
            if (xInt < 0 || xInt > turf.lineDistance(seg.network)) {
                newSegs.push(seg);
                continue;
            }

            let tmpSeg = [{
                network: turf.lineSliceAlong(seg.network, 0, xInt),
                address: {
                    type: 'Feature',
                    properties: {},
                    geometry: {
                ...
                network: turf.lineSliceAlong(seg.network, xInt, turf.lineDistance(seg.network)),
                address: {
                    type: 'Feature',
                    properties: {},
                    geometry: {
        ...
        if (newSegs.length === segs) return false; //No change between new & original segs

        console.error('----');
        console.error(JSON.stringify(segs));
        console.error(JSON.stringify(newSegs));
        return newSegs;
    }
