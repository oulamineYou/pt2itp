...
class FrequencyDistribution {
     ...
     * @returns {(string|Array<string>)} intuitive key
     */
    unmakeKey(key) {
        if (key.indexOf('|')) {
            return key.split('|');
    ...
    entries() {
        ...
        for (var [key, frequency] of this.frequencies.entries()) {
            all_entries.push([this.unmakeKey(key), frequency]);
        }
        return all_entries;
    }
    ...
    keys() {
        var all_keys = [];
        for (var [key, frequency] of this.frequencies.entries()) {
            all_keys.push(this.unmakeKey(key));
        }
        return all_keys;
    }
...
/**
 * Base class for collocation tables.
 *
...
class NGramCollocationTable {
    ...
    getExpectedValues(contingency) {
        ...
        /* Iterate over the cells of the contingency. The contingency variable
         * is an array that goes from top-left to bottom-right of a contingency
         * square (or cube). For bigrams (w1,w2), it is a two-by-two table:
         *
         *      w1               ~w1
         *      ---------------  ----------------
         ...
         *                                        TOTAL = n_all
         *
	 * We want the expected value of each cell in that table, so we need to
         * make 4 calculations. Informally, when you want the expected value of
         * cell (x,y), you do:
         *
         ...
         * multiplying using the appropriate cells for each expected value.
         ...
         *
         *   i=0    bits[1]=1      k       i&bits[j]      k&bits[j]    select k?
         *   ---    ---------      ---     ---------      ---------    ------
        ...
        for (var i=0; i < contingency.length; i++) {
            ...
            var expectedValue = productOfSums / denom;
            // console.log(`expectedValue is ${productOfSums} / ${denom} = ${expectedValue}`);
     ...
     * "Foundations of Statistical Natural Language Processing" (sec 5.3.4). This
     * explanation uses bigrams because it's easier to understand, but the
     ...
     * cell in the expectedValues table. It then sums these likelihoods and
     * multiplies them by the order of the collocation table itself.
     *
...
class BiGramCollocationTable extends NGramCollocationTable {
    ...
    score_ngrams() {
        var scores = [];
        var bigrams = this.bigram_fd.keys();
        for (var i=0; i < bigrams.length; i++) {
            var bigram = bigrams[i];
            scores.push({
                w1: bigram[0],
                w2: bigram[1],
                frequency: this.bigram_fd.absoluteFrequency(bigram),
                likelihood: this.score_ngram('likelihoodRatio', bigram)
            });
        }
        return scores;
    }
...
class TriGramCollocationTable extends BiGramCollocationTable {
    ...
    getContingency(ngram) {
        ...
        // Component frequencies
        var n_iix = this.bigram_fd.absoluteFrequency([ngram[0], ngram[1]]);
        var n_ixi = this.wildcard_fd.absoluteFrequency([ngram[0], ngram[2]]);
        var n_xii = this.bigram_fd.absoluteFrequency([ngram[1], ngram[2]]);

        var n_ixx = this.unigram_fd.absoluteFrequency(ngram[0]);
        var n_xix = this.unigram_fd.absoluteFrequency(ngram[1]);
        var n_xxi = this.unigram_fd.absoluteFrequency(ngram[2]);

        // Build contingency table
        var n_iii = this.trigram_fd.absoluteFrequency(ngram);
    ...
    score_ngrams(score_name) {
        var scores = [];
        var trigrams = this.trigram_fd.keys();
        for (var i=0; i < trigrams.length; i++) {
            var trigram = trigrams[0];
            scores.push({
                w1: trigram[0],
                w2: trigram[1],
                frequency: this.trigram_fd.absoluteFrequency(trigram),
                likelihood: this.score_ngram(score_name, trigram)
            });
        }
        return scores;
    }
