        ...
        index.getMeta(true, (err, meta) => {
            ...
            function diffName() {
                ...
                client.query('SELECT count(*) FROM address a WHERE netid IS NOT NULL', (err, res) => {
                    ...
                    const cursor = client.query(new Cursor('SELECT a._text AS atext, n._text AS ntext, ST_ASGeoJSON(ST_Centroid(a.geom)) AS center FROM address_cluster a LEFT JOIN network_cluster n ON a.netid = n.id WHERE a.netid IS NOT NULL AND a._text != any(regexp_split_to_array(n._text, \',\'))' + (argv.limit ? ' LIMIT ' + argv.limit : '') + ';'));
