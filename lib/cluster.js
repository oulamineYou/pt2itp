...
class Cluster {
    ...
    /**
     * Cluster orphan points within a given segment
    ...
    orphan(segment = 1, cb) {
        ...
        `, (err, res) => {
            if (!err) console.error(`ok - clustered addresses - seg: ${segment}`);
    ...
    network(segment = 1, cb) {
        this.pool.query(`
            ...
            COMMIT;
        `, (err, res) => {
            if (!err) console.error(`ok - clustered network - seg: ${segment}`);
        ...
        this.pool.connect((err, client, pg_done) => {
            ...
            function iterate() {
                cursor.read(100, (err, rows) => {
                    if (!rows.length) {
                        if (batch.length) { q.defer(pruneBatch, batch) };
                        q.awaitAll((err, results) => {
                            let deleteMe = results.reduce((prev, cur) => { return prev.concat(cur); }, []).join(',');
                            deleteMe = deleteMe ? deleteMe : [-1]; // if no deletions needed, use an impossible id to keep SQL valid
    ...
    collapse(callback) {
        ...
        **/
        function doStep(stepName, stepFunc, cb) {
            ...
            return stepFunc((err) => {
                if (!err)
                    console.log(`ok - collapse.${stepName} - performed ${that.mergeCount} merges, removing ${that.mergeRemoveCount} network clusters in ${Math.round(+(new Date()) - that.startTime)} ms`);
                return cb(err);
            });
        }
