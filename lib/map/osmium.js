...
function map(feat, context) {
    ...
    for (let i = 0; i < names.length; i++) {
        ...
        if (['au', 'ca', 'gb', 'nz', 'us'].indexOf(context.country) !== -1 && !highway) {
            ...
            syn = text.number_suffix(names[i].name);
            if (syn) {
                additional.push({
                    name: syn,
                    priority: -1
            ...
            syn = text.written_numeric(names[i].name);
            if (syn) {
                additional.push({
                    name: syn,
                    priority: -1
...
function alts_state_hwy(region, name) {
    ...
    if (highway) {
        ...
        //SR 123
        names.push({
            name: highway.replace(new RegExp(`^${region} `, 'i'), `SR `),
            priority: -1
        });
        ...
        //State Highway 123
        names.push({
            name: highway.replace(new RegExp(`^${region} `, 'i'), `State Route `),
            priority: -1
        });
    }
