...
function post(feat, opts) {
    ...
    for (let intersection of intersections) {
        const streets = opts.label(intersection.street);

        for (const st of streets) {
            final.push(st);
            feat.geometry.geometries[feat.geometry.geometries.length - 1].coordinates.push(intersection.geom.geometry.coordinates);
        }
    }
