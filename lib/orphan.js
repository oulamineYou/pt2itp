...
class Orphan {
    ...
    /**
     * Output all address_orphan_clusters not matched to a network_cluster
    ...
    orphan(cb) {
        ...
        this.pool.connect((err, client, done) => {
            ...
            const cursor = client.query(new Cursor(`
                SELECT _text AS address_text, ST_AsGeoJSON(geom) AS geom 
             ...
             */
            function iterate() {
                console.log('ok - in iterate:', typeof self.label);
                cursor.read(1000, (err, rows) => {
                    ...
                    rows.forEach((row) => {
                        ...
                        console.log('ok - geom:', geom);

                        const units = new Units();

                        geom.coordinates.forEach((coord) => {
                            console.log('ok - coord:', coord);
