...
function join(itps, argv) {
    ...
    for (let res of itps) {
        ...
        if (res.properties.intersections) {
            itp.properties.intersections = itp.properties.intersections.concat(res.properties.intersections);            
        }
    ...
    if (itp.properties['carmen:addressnumber'][1].length === 0) {
        ...
        itp.geometry.geometries[0].coordinates = itp.geometry.geometries[0].coordinates.map((coord) => {
