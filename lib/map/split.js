...
class SplitFeat {
    ...
    static from_id(pool, nid, cb) {
        ...
        const createJoinTableIdIndex = `
            create index if not exists address_cluster_id_to_z__id
            on address_cluster_id_to_z (id)`;
        const createJoinTableZIndex = `
            create index if not exists address_cluster_id_to_z__z
            on address_cluster_id_to_z (z)`;
        const selectSplit = `
            SELECT
                name,
                ST_AsGeoJSON(network_geom)::JSON        AS network,
                ST_AsGeoJSON(address_geom)::JSON        AS address,
                json_agg(address_props) AS address_props,
                json_agg(intersections) AS intersections
            from (
                SELECT
                    network_cluster.name || address_cluster.name    AS name,
                    network_cluster.geom        AS network_geom,
                    address_cluster.geom        AS address_geom,
                    json_build_object(
                         'id', address.id,
                ...
                FROM
                    network_cluster
                        JOIN
                    address_cluster
                            on (network_cluster.address = address_cluster.id)
                        JOIN
                    address_cluster_id_to_z
                            on (address_cluster.id = address_cluster_id_to_z.id)
                        LEFT JOIN
                    address on (address.id = address_cluster_id_to_z.z)
                        LEFT JOIN
                    intersections on (intersections.a_id = network_cluster.id or
                                      intersections.b_id = network_cluster.id)
                WHERE
                    network_cluster.id = ${nid}
                ) x
            group by name, network_geom, address_geom
        `;
                    ...
                    client.query(createJoinTableIdIndex, (err, res) => {
                        ...
                        client.query(createJoinTableZIndex, (err, res) => {
                            ...
                            client.query('COMMIT', (err) => {
                                if (err) return cb(err);
                                client.query(selectSplit, (err, res) => {
                                    if (err) return cb(err);
                                    ...
                                    const feat = new SplitFeat(nid, res.name,
                                        lookup, res.network, res.address, res.intersections);
...
class Split {
    ...
    split(feat, cb) {
        ...
        let intersections = feat.intersections || [];
        ...
        if (Misc.hasDupAddressWithin(props.map((prop) => { return prop.number }), coords)) {
            let tmpSegs = this.distribute(network, coords, props, intersections);

            let potentialSegs = Cluster.break(tmpSegs, feat.id);

            if (potentialSegs) {
                segs = potentialSegs.map((seg) => {
                    return this.distribute(this.explode.split(seg.network), seg.address, seg.number, seg.intersections)
                });
            } else {
                segs = [ this.distribute(this.explode.split(network), coords, props, intersections) ];
            }
        } else {
            segs = [ this.distribute(this.explode.split(network), coords, props, intersections) ];
        }
    ...
    distribute(network, coords, props, intersections) {
        ...
        for (let intsec of intersections) {
            let currentMatch = {
                dist: Infinity,
                seg: false
            };
            ...
            for (let seg of segs) {
                let dist = turf.distance(turf.pointOnLine(seg.network.geometry, intsec.geom), intsec.geom);
