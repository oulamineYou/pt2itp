...
function extractTextField(cc, type, callback) {
    ...
    var country = cc.split('_')[0];
    if (cc.indexOf('_') > -1) {
        var region = cc.split('_')[1];
...
function frequencyDistributionMunger(textfields, callback) {
    ...
    for (var textField in textfields) {
        var tokens = tokenize.main(textfields[textField]);
...
/**
 * Writes an array of scored ngrams to a CSV file
 *
 * @param {Array<Object>} scores - An array of scored ngrams
 */
function writeToCSV(scores, filepath) {
var scoreFile = fs.createWriteStream(filepath);
    csv
