        ...
        if (Misc.hasDupAddressWithin(props.map((prop) => { return prop.number }), coords)) {
            ...
            if (potentialSegs) {
                segs = potentialSegs.map((seg) => {
                    return this.distribute(this.explode.split(seg.network, seg.intersections), seg.address, seg.number, seg.intersections)
                });
            } else {
                segs = [ this.distribute(this.explode.split(network, intersections), coords, props, intersections) ];
            }
        } else {
            segs = [ this.distribute(this.explode.split(network, intersections), coords, props, intersections) ];
        }
