...
function test(argv, cb) {
    ...
    if (argv.tokens) {
        argv.tokens = tokenize.createReplacer(argv.tokens.split(','));
    } else {
        argv.tokens = {};
    }
                            ...
                            for (let ntext of row.n_name) {
                                for (let atext of row.a_name) {
                                    ...
                                    if (['default', 'token'].indexOf(argv.name) !== -1) {
                                        ...
                                        ntext = tokenize.main(tokenize.replaceToken(tokenRegex, ntext), argv.tokens, true)[tokenKey].join(' ');
                                        atext = tokenize.main(tokenize.replaceToken(tokenRegex, atext), argv.tokens, true)[tokenKey].join(' ');
                                    }
