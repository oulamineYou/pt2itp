    ...
    address(cb) {
        this.pool.query(`
            ...
            INSERT INTO address_cluster (name, geom, netid)
                SELECT
                    JSON_Agg(a.name ORDER BY ST_NumPoints(geom) DESC),
                    ST_Multi(ST_CollectionExtract(ST_Collect(a.geom), 1)) AS geom,
                    a.netid
                FROM (
                    SELECT
                        json_array_elements(MAX(name::TEXT)::JSON) AS name,
                        netid,
                ...
                that.pool.connect((err, client, pg_done) => {
                    ...
                    const processCursor = client.query(new Cursor(`
                        SELECT
                             ...
                             JSON_Agg(n2.name, ORDER BY n2.id ASC) AS cluster_texts,
        ...
        this.pool.query(`SELECT ST_AsGeoJSON(geom) AS geom FROM network WHERE id = ${id}`, (err, res) => {
            ...
            this.pool.query(`SELECT name FROM address WHERE ST_Within(geom, ST_SetSRID(ST_GeomFromGeoJSON('${JSON.stringify(buff)}'), 4326))`, (err, res) => {
                ...
                for (let res_it = 0; res_it < res.rows.length; res_it++) {
                    ...
                    if (res.rows[res_it].name.display) {
                        let text = name[0].display;
                ...
                if (res.rows.length == 0 || !namelist.length) {
                    ...
                    new_name = namelist[0].name;

                    // titlecase / run label func if zero case changes -- but only for _text
                    if ((k === 2) && (numCaseChanges(new_name.display) === 0))
