...
function writeUnigram(argv, pool, bigrams, cb) {
    ...
    let popQ = new Queue(1);
    function writeUnigramToCSV(word, frequency, relativeFrequency, callback) {
    ...
    function writeUnigramToTable(type, word, frequency, relativeFrequency, callback) {
        pool.query({
    ...
    for (let [word, frequency] of bigrams.unigram_fd.entries()) {
        ...
        popQ.defer(writeUnigramToCSV, word, frequency, relativeFrequency);

        // Insert the frequency to table
        popQ.defer(writeUnigramToTable, argv.type, word, frequency, relativeFrequency);
    }
    popQ.await((err) => {
        cb(err);
    });
