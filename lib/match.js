...
function match(min, max, cb) {
    ...
    `, (err, res) => {
        ...
        pool.connect((err, trans, pg_done) => {
            ...
            `, (err, res) => {
                ...
                for (let lRes of linkerRes) {
                    dbQ.defer(commit, trans, lRes.id, lRes.nets);
                }
         ...
         * @param {Array} nets Array of potential valid network matches - dist attribute will pick top result
         * @param {function} done Callback in form (err, res)
         * @return {function} callback in form (err, res)
         */
        function commit(trans, id, nets, done) {
