...
class Cluster {
    ...
    **/
    collapse(callback) {
        ...
        collapseQ.awaitAll(() => { console.log(`ok - collapsed network clusters in ${Math.round(+(new Date()) - that.startTime)} ms`); process.exit(1); });
                        ...
                        processCursor.read((err, rows) => {
                            ...
                            if (rows.length) {
                                for (let row of rows) {
                                    ...
                                    let parent = clusters.shift();
                                    let collapseMe = [];
                                    for (let child of clusters) {
                                        let overlapLength = networkLength(child.source_ids.map((x) => { return parent.source_ids.indexOf(x) !== -1; }));
                                        // merge if overlapping portion is >=95% of parent length
                                        if (((overlapLength / parent.network_length) > 0.95) && (processedIds.indexOf(child.id) === -1)) {
                                            collapseMe.push(child.id);
                                            processedIds.push(child.id);
                                        }
                                    }
                                    if (collapseMe.length > 0)
                                        q.defer(collapseNetworkClusters, parent.id, collapseMe);
                                }
