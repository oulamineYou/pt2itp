...
class Cluster {
     ...
     */
    address(cb) {
        // maybe add a column which is the number of addresses with that text and network id :grimace:
        // as it is now running the same select for all three columns; waste of time
        this.pool.query(`
            BEGIN;

            INSERT INTO address_cluster (text, _text, text_tokenless, geom, netid)
                SELECT
                    Array_Agg(DISTINCT a.text 
                        ORDER BY COUNT(
                            SELECT * FROM address AS ad WHERE ad.text = a.text AND ad.netid = a.netid )),
                    Array_Agg(DISTINCT a._text 
                        ORDER BY COUNT(
                            SELECT * FROM address AS ad WHERE ad._text = a._text AND ad.netid = a.netid )),
                    Array_Agg(DISTINCT a.text_tokenless 
                        ORDER BY COUNT(
                            SELECT * FROM address AS ad WHERE ad.text_tokenless = a.text_tokenless AND ad.netid = a.netid )),
