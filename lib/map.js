...
function main(argv, cb) {
    ...
    function collapse() {
        // setup
        /*
        CREATE TABLE address_cluster_point_count AS
        SELECT
            a.id,
            COUNT(a.id) AS total
        FROM
        ...
        ALTER TABLE network_cluster ADD COLUMN point_count NUMERIC;

        UPDATE network_cluster n SET point_count=acpc.total FROM address_cluster_point_count acpc WHERE n.address IS NOT NULL AND acpc.id=n.address;
        */
        ...
        SELECT
            n1.id,
            MAX(n1._text),
            STRING_AGG(n2.id::text, ',' ORDER BY n2.id ASC) AS contained_cluster_ids,
            STRING_AGG(n2._text::text, ',' ORDER BY n2.id ASC) AS contained_cluster_names,
            STRING_AGG(COALESCE(n2.address,-1)::text, ',' ORDER BY n2.id ASC) AS contained_cluster_address,
            STRING_AGG(COALESCE(n2.point_count,0)::text, ',' ORDER BY n2.id ASC) AS address_cluster_point_count
        FROM
