...
function start(callback) {
    ...
    rl.on('line', (data) => {
        ...
        try {
            if (opts.map)
                features = opts.map(JSON.parse(data));
            else
        ...
        for (let feat of features) {
            ...
            if (opts.type === 'address') {
                ...
                rl.output.write(`${str(feat.properties.street)}|${str(feat.properties.streetTokenless)}|${str(feat.properties._text)}|${JSON.stringify(str(feat.geometry.coordinates[0]))}|${JSON.stringify(str(feat.geometry.coordinates[1]))}|${str(feat.properties.number)}|${feat.properties.source}\n`);
            } else {
                if (!feat.properties.id) feat.properties.id = 0;
                // when ID is available, use it for the linestring's z coord. necessary for passing data past st_clusterwithin()
                ...
                rl.output.write(`${feat.properties.id}|${str(feat.properties.street)}|${str(feat.properties.streetTokenless)}|${str(feat.properties._text)}|${str(JSON.stringify(feat.geometry))}\n`);
            }
        }
    });
}
