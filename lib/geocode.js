...
function isPass(query, pt, res, opts = {}, done) {
    ...
    let cleanQuery = tokenize.main(diacritics(query), opts.tokens);
    // let cleanQuery = tokenize.replaceToken(tokenRegex, tokenize.main(diacritics(query), opts.tokens).join(' '));
    ...
    let matched = tokenize.main(diacritics(name), opts.tokens)
    // let matched = tokenize.replaceToken(tokenRegex, tokenize.main(diacritics(name), opts.tokens).join(' '));
    ...
    cleanQuery = tokenize.replaceToken(tokenRegex, cleanQuery.join(' '));
    matched = tokenize.replaceToken(tokenRegex, matched.join(' '));

    if (matchedCount / cleanQuery.length < 0.70) {
        if (opts.stats) opts.stats.fail++;
        return done(null, ['TEXT', { query: cleanQuery, queryPoint: pt.join(','), addressText: matched, returnedPoint: res.features[0].geometry.coordinates ? res.features[0].geometry.coordinates.join(',') : '' }]);
