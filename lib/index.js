    ...
    init(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE address (id SERIAL, text JSON, number NUMERIC, lon TEXT, lat TEXT, source TEXT, output BOOLEAN, geom GEOMETRY(POINTZ, 4326), netid BIGINT);
                CREATE INDEX ON address (id);

                DROP TABLE IF EXISTS network;
                CREATE TABLE network (id BIGINT, text JSON, named BOOLEAN, geomtext TEXT, geom GEOMETRY(LINESTRINGZ, 4326), network_length NUMERIC);
                ...
                CREATE TABLE network_cluster(ID SERIAL, text TEXT, _text TEXT, text_tokenless TEXT, priority INTEGER, geom GEOMETRY(GEOMETRYZ, 4326), address BIGINT, source_ids BIGINT[]);
                    ...
                    addrQ.defer((output, done) => {
                        self.pool.connect((err, client, pgdone) => {
                            ...
                            const cpStream = client.query(pgcopy(`
                                COPY address (text, lon, lat, number, source, output)
                                    FROM STDIN
                                    WITH
                                        CSV
                                        DELIMITER '|'
                ...
                for (let child of nursery) {
                    netQ.defer((output, done) => {
                        self.pool.connect((err, client, pgdone) => {
                            ...
                            const cpStream = client.query(pgcopy(`
                                COPY network (id, text, geomtext, network_length)
                                    FROM STDIN
                                    WITH
                                        CSV
                                        DELIMITER '|'
