...
function interpolize(text, splits, argv = {}) {
    ...
    //Store address points to be output at end but not used for ITP processing
    let quarantined = [];
    ...
    if (max === -1 && min === Infinity) return join(itps, quarantined, argv);
    let d = diff(max, min);
    ...
    return join(itps, quarantined, argv);
}

function join(itps, quarantined, argv) {
    let itp = {
        type: 'Feature',
        properties: {
    ...
    for (let addr of quarantined) {
        itp.properties['carmen:addressnumber'][1].push(String(addr.number));
        itp.geometry.geometries[1].coordinates.push(addr.geometry.coordinates);
    }
