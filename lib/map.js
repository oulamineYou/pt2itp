...
function main(argv, cb) {
     ...
     */
    function matcher() {
        return intersection(); //TODO REMOVE THIS BEFORE MERGING
    ...
    async function intersection() {
        ...
        try {
            await new (require('./util/intersections'))({
                pool: poolConf
            }).generate();
            ...
            let cpu_spawn = Math.min(Math.ceil(intersectionNum / 5000), CPUS); //number of 5000 groups or # of CPUs, whichever is smaller
            let nursery = [];

            let intersectionNum_it = 1; //Postgis SERIAL begins at 1

            while (cpu_spawn--) {
                let child = CP.fork(path.resolve(__dirname, './util/intersections'), {
                    stdio: ['pipe', 'pipe', 'pipe', 'ipc']
                });
                ...
                child.on('message', (message) => {
                    ...
                    if (intersectionNum_it >= intersectionNum) {
                        nursery[message.id].kill();
                        ...
                        if (!active) {
                            console.error('ok - intersections output');
                            return finalize();
                        }
                    } else {
                        let max = intersectionNum_it = intersectionNum_it + 5000;
                        let min = max - 5000;
