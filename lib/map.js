...
const Intersections = require('./util/intersections');
...
function main(argv, cb) {
    ...
    const intersections = new Intersections(pool);
    ...
    async function intersection() {
        ...
        try {
            await intersections.generate();

            console.error('ok - generated intersections');

            const res = await client.query(`SELECT MAX(id) AS max FROM intersections`);
