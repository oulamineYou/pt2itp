        ...
        for (let seg_it = 0; seg_it < segs.length; seg_it++) {
            ...
            for (let addr_it = 0; addr_it < seg.number.length; addr_it++) {
                ...
                let res = {
                    ...
                    original: seg.number[addr_it].num,
            ...
            const breaks = []; //Array indices to break on

            // Address Cliff Detection
            // 1, 2, 3, 1, 2, 3
            ...
            for (let brk_it = 0; brk_it < breaks.length; brk_it++) {
                ...
                const brk_dist_start = isFirst ? 0 : dist[brk - 1].distOnLine + (dist[brk].distOnLine - dist[brk - 1].distOnLine) / 2;
                const brk_dist_end = dist[brk].distOnLine + (dist[brk + 1].distOnLine - dist[brk].distOnLine) / 2;

                newSegs.push({
                    address: turf.feature(turf.multiPoint(dist.slice(isFirst ? 0 : breaks[brk_it - 1] + 1, brk + 1).map((d) => {
                        return d.geometry.geometry.coordinates;
                    }))),
                    number: dist.slice(isFirst ? 0 : breaks[brk_it - 1] + 1, brk + 1).map((d) => {
                        return d.original;
                    }),
                    network: turf.lineSliceAlong(seg.network, brk_dist_start, brk_dist_end)
                });
            }

            //Push remaining portion of segment
            newSegs.push({
                address: turf.feature(turf.multiPoint(dist.slice(breaks[breaks.length - 1]).map((d) => {
                    return d.geometry.geometry.coordinates;
                }))),
                number: dist.slice(breaks[breaks.length - 1]).map((d) => {
                    return d.original;
                }),
                network: turf.lineSliceAlong(seg.network,  dist[breaks[breaks.length - 1]].distOnLine + (dist[breaks[breaks.length - 1] + 1].distOnLine - dist[breaks[breaks.length - 1]].distOnLine) / 2, turf.length(seg.network))
            });
        }

        //TODO: RETURN ORPHAN SEGS
        return newSegs;
    }
