...
function match(min, max, cb) {
        ...
        SELECT
            ...
            (Array_Agg(
                JSON_Build_Object(
                    'id', nc.id,
                    'name', nc.name::JSON,
                    'dist', ST_Distance(ST_ClosestPoint(nc.geom, a.geom), a.geom)
                )
                ORDER BY ST_Distance(ST_ClosestPoint(nc.geom, a.geom), a.geom)
            ))[:10] AS nets
        FROM
            address a
    ...
    `, (err, res) => {
        ...
        for (let row of res.rows) {
            ...
            let nets = linker(row.name, row.nets);
