    ...
    init(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE network (id BIGINT, text TEXT, text_tokenless TEXT, _text TEXT, named BOOLEAN, priority NUMERIC DEFAULT 0, geomtext TEXT, geom GEOMETRY(LINESTRINGZ, 4326), network_length NUMERIC);
                ...
                for (let child of nursery) {
                    netQ.defer((output, done) => {
                        self.pool.connect((err, client, pgdone) => {
                            ...
                            const cpStream = client.query(pgcopy(`COPY network (id, text, text_tokenless, _text, priority, geomtext, network_length) FROM STDIN WITH CSV DELIMITER '|' QUOTE E'\b' NULL AS ''`));
                            const fsStream = fs.createReadStream(output);
