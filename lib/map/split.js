    ...
    static from_id(pool, nid, cb) {
        pool.query(`
            SELECT
                ...
                COALESCE(
                    json_agg(address_objs) FILTER (WHERE address_objs IS NOT NULL),
                    '[]'::JSON) AS address_props,
                COALESCE(
                    json_agg(intersection_objs) FILTER (WHERE intersection_objs IS NOT NULL),
                    '[]'::JSON) AS intersections
            FROM (
                SELECT
                    ...
                    case when address.id is not null then
                        ...
                        )  else null end AS address_objs,
                    case when intersections.id is not null then
                        json_build_object(
                            'id', intersections.id,
                        ...
                        ) else null end as intersection_objs
                FROM
                    network_cluster
                        JOIN
                    address_cluster
        ...
        for (let intsec of intersections) {
            ...
            for (let seg of segs) {
                try {
                    ...
                    console.error(seg.network.geometry);
                    console.error(intsec.geom);
                }
            }
