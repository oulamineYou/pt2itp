...
function main(argv, cb) {
            ...
            client.query(`SELECT id FROM network_cluster WHERE text != '' AND address IS NOT NULL;`, (err, res) => {
                ...
                while (cpu_spawn--) {
                    ...
                    if (!argv.label && argv.tokens && (argv.tokens.indexOf('en') === -1))
                        console.error('WARN: using titlecase behavior, which is currently English-only, on non-English data');
                    child.send(opts);
                }
            });
        });
    ...
    function orphanAddr() {
        let label;
        if (!argv.label)
            label = require('./label/titlecase')();
        else
            label = require('./label/' + argv.label)();
        ...
        pool.connect((err, client, pg_done) => {
            ...
            client.query(`SELECT count(*) FROM address_cluster a LEFT JOIN network_cluster n ON a.id = n.address WHERE n.address IS NULL;`, (err, res) => {
                ...
                const cursor = client.query(new Cursor(`SELECT a._text AS address_text, n._text as network_text, ST_AsGeoJSON(a.geom) AS geom FROM address_cluster a LEFT JOIN network_cluster n ON a.id = n.address WHERE n.address IS NULL;`));

                return iterate();

                /**
                 * Iterate over orphaned addrs using PG Cursor to prevent excessive memory use
                 */
                function iterate() {
                    cursor.read(100, (err, rows) => {
                        if (err) return cb(err);
