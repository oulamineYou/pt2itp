...
class Explode {
    ...
    /**
     * Split LineStrings into segments based on addresses - ie if it detects multiple 1000+ blocks break into blocks
     * and/or have a max LineString distance of 500m
     *
     * Linesegments have a split distance min of 100m - Input linesegments < 100m will be retained but linestrings will not be split to less
     *
     * @param {Object} streets GeoJSON FeatureCollection of streets to split
     * @param {Array} coords MultiPoint coordinate array
     * @param {Array} numbers Parallel array of civic numbers
     * @returns {Array} GeoJSON FeatureCollection
     */
    split(streets, addresses = [], numbers = []) {
        if (addresses.length && numbers.length) {
            let min = _.min(numbers);
            let max = _.max(numbers);

            if (max - min > 1000) {
                let rndMin = misc.toLowest(min, 1000);
                let rndMax = misc.toHighest(max, 1000);

                let sorted = _.clone(numbers).sort();
        ...
        for (let feat of streets.features) {
            ...
            for (let dist_it = 0; dist_it < dist; dist_it++) {
                distRes.features.push(turf.lineSliceAlong(feat, dist_it, dist_it + 0.5, 'kilometers'));
            }
        }
