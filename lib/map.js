...
function main(argv, cb) {
    ...
    if (argv['map-network']) argv['map-network'] = path.resolve(__dirname, './input/', argv['map-network'] + '.js');
    if (argv['map-address']) argv['map-address'] = path.resolve(__dirname, './input/', argv['map-address'] + '.js');
...
>>>>>>> 6954534c53b26629990b7f70da1683a6dee8c431
    ...
    async function intersection() {
        ...
        if (!argv.intersections) return finalize();

        try {
            const intsec = new (require('./util/intersections'))({
                pool: poolConf
            });
            ...
            const res = await pool.query(`SELECT MAX(id) AS max FROM intersections`);
            if (!res.rows.length || res.rows[0].max <= 0) throw new Error('address has no addresses to match!');

            const intersectionNum = res.rows[0].max;
            const bar = new Prog('ok - Generating Intersections [:bar] :percent :etas', {
                complete: '=',
                incomplete: ' ',
                width: 20,
                total: intersectionNum + 1
            });
            bar.tick(1); //Show progress bar

            let intersectionNum_it = 1; //Postgis SERIAL begins at 1

            process_int();

            /**
             * Generate a single batch of intersections
             */
            async function process_int() {
                let max = intersectionNum_it = intersectionNum_it + 1000;
                let min = max - 1000;

                const q = new Queue(1);
                for (let feat of await intsec.name({ min: min, max: max, country: argv.country })) {
                    q.defer((feat, done) => { output.write(JSON.stringify(feat) + '\n', done) }, feat);
                }

                q.awaitAll(() => {
                    bar.tick(max - min);

                    if (intersectionNum_it < intersectionNum) return process.nextTick(process_int);
                    return finalize();
                });
            }
        } catch (err) {
