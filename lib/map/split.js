...
class SplitFeat {
     ...
     */
    constructor(name, props, network, address) {
        this.name = name;
    ...
    static from_id(pool, nid, cb) {
        ...
        `, (err, res) => {
            ...
            const feat = new SplitFeat(res.name, lookup, res.network, res.address);
...
class Split {
    ...
    split(feat, cb) {
        ...
        //Parallel array to coords that stores properties
        let props = [];

        //Sort coords for consistent input into interpolate
        const coords = res.address.coordinates.sort((a, b) => {
            if (a[2] > b[2]) return 1;
            if (a[2] < b[2]) return -1;
    ...
    if (Array.isArray(message)) {
        ...
        for (let nid of message) {
            ...
            splitQ.defer((nid, done) => {
                SplitFeat.from_id(nid, (err, feat) => {
