    ...
    init(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE network (id BIGINT, segment BIGINT, text TEXT, text_tokenless TEXT, _text TEXT, named BOOLEAN, geomtext TEXT, geom GEOMETRY(LINESTRINGZ, 4326));
            ...
            if (type === 'address') {
                ...
                for (let child of nursery) {
                    netQ.defer((output, done) => {
                        self.pool.query(`
                            COPY network (id, text, text_tokenless, _text, geomtext)
                            FROM '${output}' WITH CSV DELIMITER '|' QUOTE E'\b' NULL AS '';
                        `, (err) => {
                            if (err) return done(err);
                            fs.unlink(output, (err) => {
