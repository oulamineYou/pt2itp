...
function main(argv, cb) {
    ...
    if (argv['map-network']) argv['map-network'] = path.resolve(__dirname, './inputs/', argv['map-network'] + '.js');
    if (argv['map-address']) argv['map-address'] = path.resolve(__dirname, './inputs/', argv['map-address'] + '.js');
    ...
    async function intersection() {
        ...
        try {
            ...
            async function process_int() {
                ...
                const q = new Queue(1);
                const feats = await intsec.batch({ min: min, max: max });

                for (let feat of await intsec.name(feats.rows, { country: argv.country })) {
