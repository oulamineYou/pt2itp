...
class Index {
    ...
    init(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE persistent (id BIGINT, version BIGINT, name JSONB, number TEXT, props JSONB, lon TEXT, lat TEXT, geom GEOMETRY(POINT, 4326));
    ...
    */
    copy(file, opts = {}, cb) {
         ...
         */
        function copyRes() {
            console.error(`ok - standardized persistent address input data`)
            console.error(`ok - importing persistent address data`);
                ...
                addrQ.defer((output, done) => {
                    self.pool.connect((err, client, pgdone) => {
                        ...
                        const cpStream = client.query(pgcopy(`
                            COPY persistent (id, version, name, number, props, lon, lat)
                                FROM STDIN
                                WITH
                                    CSV
                                    DELIMITER '|'
    ...
    optimize(cb) {
        ...
        this.pool.query(`
            ...
            UPDATE persistent SET geom = ST_SetSRID(ST_MakePoint(substring(lon from 1 for 10)::NUMERIC, substring(lat from 1 for 10)::NUMERIC), 4326);

            ALTER TABLE persistent DROP COLUMN lat;
            ALTER TABLE persistent DROP COLUMN lon;

            CREATE INDEX number_idx ON persistent (number);
            ...
            CLUSTER persistent USING persistent_gix;
            ANALYZE persistent;
