    ...
    init_conflate(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE persistent (id SERIAL, name JSONB, number TEXT, props JSONB, lon TEXT, lat TEXT, geom GEOMETRY(POINT, 4326));
    ...
    init(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE address (id SERIAL, name JSONB, number TEXT, encoded NUMERIC, lon TEXT, lat TEXT, source TEXT, output BOOLEAN, geom GEOMETRY(POINTZ, 4326), netid BIGINT);
                CREATE INDEX ON address (id);

                DROP TABLE IF EXISTS network;
                CREATE TABLE network (id BIGINT, name JSONB, geomtext TEXT, geom GEOMETRY(LINESTRINGZ, 4326), network_length NUMERIC);
                CREATE INDEX ON network (id);

                DROP TABLE IF EXISTS address_cluster;
                CREATE TABLE address_cluster(ID SERIAL, name JSONB, geom GEOMETRY(MULTIPOINTZ, 4326), netid BIGINT);
                    ...
                    addrQ.defer((output, done) => {
                        self.pool.connect((err, client, pgdone) => {
                            ...
                            const cpStream = client.query(pgcopy(`
                                COPY address (name, lon, lat, number, encoded, source, output)
                                    FROM STDIN
                                    WITH
                                        CSV
                                        DELIMITER '|'
    ...
    optimize(cb) {
        ...
        optimizeQ.defer((done) => {
            this.pool.query(`
                ...
                UPDATE address SET geom = ST_SetSRID(ST_MakePoint(substring(lon from 1 for 10)::NUMERIC, substring(lat from 1 for 10)::NUMERIC, encoded), 4326);
