...
class Compare {
    ...
    constructor(o) {
        ...
        this.opts.read = fs.createReadStream(this.opts.read);
    ...
    read(cb) {
        const rl = Readline.createInterface({
            ...
            `, (err, res) => {
                ...
                return this.compare(feat, res.rows);
            });
        }).on('close', (close) => {
            return cb();
        });
    ...
    compare(feat, rows) {
        ...
        if (rows.length === 0) return this.opts.output.write(JSON.stringify(create(feat)) + '\n');
        ...
        for (let r of res.rows) {
            ...
            for (let potential of potentials) {
                if (known.indexOf(potential) !== -1) {
                    ...
                    if (m) return this.opts.output.write(JSON.stringify(m) + '\n');
                }
            }
        }
    }
    ...
    modify(known, potential) {
        ...
        for (let pname of potential.properties.street) {
            ptoken = tokenize.replaceToken(tokenRegex, tokenize.main(pname.display, this.opts.tokens, true).tokens.join(' '));

            let exists = false;
            for (let kname of known.properties.street) {
                ktoken = tokenize.replaceToken(tokenRegex, tokenize.main(kname.display, this.opts.tokens, true).tokens.join(' '));
