    ...
    init(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE address (id SERIAL, name JSONB, number NUMERIC, lon TEXT, lat TEXT, source TEXT, output BOOLEAN, geom GEOMETRY(POINTZ, 4326), netid BIGINT);
                CREATE INDEX ON address (id);

                DROP TABLE IF EXISTS network;
                CREATE TABLE network (id BIGINT, name JSONB, named BOOLEAN, geomtext TEXT, geom GEOMETRY(LINESTRINGZ, 4326), network_length NUMERIC);
                    ...
                    addrQ.defer((output, done) => {
                        self.pool.connect((err, client, pgdone) => {
                            ...
                            const cpStream = client.query(pgcopy(`
                                COPY address (name, lon, lat, number, source, output)
                                    FROM STDIN
                                    WITH
                                        CSV
                                        DELIMITER '|'
