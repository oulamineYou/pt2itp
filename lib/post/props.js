...
function post(feat, opts) {
    ...
    for (let desired_prop of desired_props) {
        ...
        for (let val_it = 0; val_it < current_vals.length; val_it++) {
            if (current_vals[val_it] !== max.occurance) {
                if (!feat.properties['carmen:addressprops']) feat.properties['carmen:addressprops'] = {};
                if (!feat.properties['carmen:addressprops'][desired_prop]) feat.properties['carmen:addressprops'][desired_prop] = {};
