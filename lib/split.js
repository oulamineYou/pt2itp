 ...
 * @param {Object} GeoJSON FeatureCollection of network segments
 * @param {Array} Coordinates for address points
 * @param {Array} Corresponding civic numbers - parallel to coordinates array
...
function distribute(network, coords, number) {
    ...
    for (let it = 0; it < addressCluster.length; it++) { segs.push({
...
function split(nid, cb) {
    ...
    `, (err, res) => {
        ...
        if (misc.hasDupAddressWithin(number, coords)) {
            let tmpSegs = distribute(network, coords, number);

            potentialSegs = cluster.break(tmpSegs);
