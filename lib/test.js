...
function test(argv, cb) {
    ...
    const cursor_it = 25; //Number of rows to grab at a time from postgres;
    ...
    pool.connect((err, client, pg_done) => {
        index.getMeta(true, (err, meta) => {
            ...
            function matched() {
                ...
                client.query('SELECT count(*) FROM address_cluster;', (err, res) => {
                    if (err) return cb(err);

                    const cursor = client.query(new Cursor('SELECT a._text, ST_AsGeoJSON(a.geom) AS geom FROM address_cluster a' + (argv.limit ? ' LIMIT ' + argv.limit : '') + ';'));
            ...
            function unmatched() {
                ...
                client.query('SELECT count(*) FROM address_orhpan_cluster;', (err, res) => {
                    if (err) return cb(err);
            ...
            function diffName() {
                ...
                client.query('SELECT count(*) FROM address a LEFT JOIN network_cluster n ON a.nid = n.address WHERE n.address IS NOT NULL AND n._text != a._text;', (err, res) => {
                    if (err) return cb(err);
