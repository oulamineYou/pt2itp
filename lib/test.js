        ...
        index.getMeta(true, (err, meta) => {
            ...
            function matched() {
                ...
                client.query(`SELECT count(*) FROM address WHERE netid IS NOT NULL ${limitClause}`, (err, res) => {
                    ...
                    const cursor = client.query(new Cursor(`SELECT a.name, ST_AsGeoJSON(a.geom) AS geom, a.number FROM address a WHERE netid IS NOT NULL ${limitClause} ;`));
                    ...
                    function iterate() {
                        cursor.read(cursor_it * 100, (err, rows) => {
                            ...
                            for (let row of rows) {
                                ...
                                }, `${units.decode(row.number).num} ${row.name[0].display}`, {
            ...
            function unmatched() {
                ...
                client.query(`SELECT count(*) FROM address_orphan_cluster a ${limitClause}`, (err, res) => {
                    ...
                    const cursor = client.query(new Cursor(`SELECT a.name, ST_AsGeoJSON(ST_PointOnSurface(a.geom)) AS geom FROM address_orphan_cluster a ${limitClause} ;`));
                    ...
                    const cursor = client.query(new Cursor(`
                        ...
                        FROM (
                            ...
                            FROM (
                                SELECT
                                    a.id AS id,
                                    jsonb_array_elements(a.name) ->> 'display' AS a_name,
                                    jsonb_array_elements(n.name) ->> 'display' AS n_name,
                    ...
                    function iterate() {
                        cursor.read(cursor_it, (err, rows) => {
                            ...
                            if (!rows.length) {
                                ...
                                let totalErrors = Object.keys(resultTypeTotals).reduce((prev, cur) => { return prev + resultTypeTotals[cur]; }, 0);
                                ...
                                for (let errType of errTypes) {
                                    ...
                                    let pctOfErrors = (100 * resultTypeTotals[errType] / totalErrors).toFixed(1).toString();
                                    let pctOfTotal = (100 * resultTypeTotals[errType] / totalAddresses).toFixed(1).toString();
                                ...
                                for (let ntext of row.n_name) {
                                    for (let atext of row.a_name) {
                                        ...
                                        if (['default', 'token'].indexOf(argv.name) !== -1) {
                                            ...
                                            ntext = tokenize.replaceToken(tokenRegex, diacritics(tokenize.main(ntext, meta.tokens, true)[tokenKey].join(' ')));
                                            atext = tokenize.replaceToken(tokenRegex, diacritics(tokenize.main(atext, meta.tokens, true)[tokenKey].join(' ')));
                                        }
                                        ...
                                        //     const tokenKey = argv.name === 'default' ? 'tokenless' : 'tokens';
                                        //
                                        //     // normalizedNtext = tokenize.replaceToken(tokenRegex, diacritics(tokenize.main(normalizedNtext, meta.tokens, true)[tokenKey].join(' ')));
                                        //     // normalizedAtext = tokenize.replaceToken(tokenRegex, diacritics(tokenize.main(normalizedAtext, meta.tokens, true)[tokenKey].join(' ')));
                                        //
                                        // }

                                        if (!hard_mismatch && ntext !== atext) hard_mismatch = { n: ntext, a: atext };

                                        let nfuzzed = ntext.split(/\s+/).map(singularize).map(metaphone).join('');
                                        let afuzzed = atext.split(/\s+/).map(singularize).map(metaphone).join('');
