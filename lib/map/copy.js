    ...
    rl.on('line', (data) => {
        ...
        for (let feat of features) {
            ...
            if (opts.type === 'address' && texts.length == 1 && !texts[0].display.trim()) {
                ...
                if (texts[0].priority === texts[1].priority) {
                    if (opts.error) process.stderr.write(`Must have a single highest priority feature\t${data}\n`);
                    return;
                }
            }
            ...
            for (let text of texts) {
                ...
                text.tokenized = tokenize.replaceToken(tokenRegex, tokens.tokens.join(' ')); // The street is standardized and it what is used to compare to the address cluster
                text.tokenless = tokens.tokenless.join(' '); // we will also use the tokenless form during the linker phase
            ...
            if (opts.type === 'address') {
                ...
                encoded = units.encode(feat.properties.number, {
                ...
                const props = {};
                for (let prop of Object.keys(feat.properties)) {
                    if (['number', 'street', 'source', 'output'].includes(prop)) continue;
                    props[prop] = feat.properties[prop];
                }
