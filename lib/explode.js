...
class Split {
     ...
     * @param {boolean} settings.noDistance disables ~1km max LineString distance
     */
    constructor(settings = {}) {
        this.settings = settings;

        if (!this.settings.degTolerance) this.settings.degTolerance = 45;
        if (!this.settings.noDistance) this.settings.noDistance = false;
            ...
            if (streets.features[feat_it].geometry.type !== 'MultiLineString') {
                ...
                for (let line_it = 0; line_it < streets.features[feat_it].geometry.coordinates.length; line_it++) {
                    ...
                    if (this.settings.noIntersect || !intersects) {
                        ...
                        // <-c- . -w-> && <-w- . -c->
                        if (_.isEqual(working[0],coords[0]) && (wsBearing > 0 ? btn(csBearing + 180, wsBearing, this.settings.degTolerance) : btn(csBearing - 180, wsBearing, this.settings.degTolerance))) {
                            working = streets.features[feat_it].geometry.coordinates.splice(line_it, 1)[0].reverse().concat(working);
                            line_it = -1;

                        // -c-> . -w-> && <-w- . <-c-
                        } else if (_.isEqual(working[0],coords[coords.length-1]) && btn(ceBearing, wsBearing, this.settings.degTolerance)) {
                            working = streets.features[feat_it].geometry.coordinates.splice(line_it, 1)[0].concat(working);
                            line_it = -1;

                        // -w-> . -c-> && <-c- . <-w-
                        } else if (_.isEqual(working[working.length-1],coords[0]) && btn(csBearing, weBearing, this.settings.degTolerance)) {
                            working = working.concat(streets.features[feat_it].geometry.coordinates.splice(line_it, 1)[0]);
                            line_it = -1;

                        // -w-> . <-c- && -c-> . <-w-
                        } else if (_.isEqual(working[working.length-1],coords[coords.length-1]) && (weBearing > 0 ? btn(ceBearing + 180, weBearing, this.settings.degTolerance) : btn(ceBearing - 180, weBearing, this.settings.degTolerance))) {
