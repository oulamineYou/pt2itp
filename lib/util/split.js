...
function split(nid, cb) {
    ...
    `, (err, res) => {
        ...
        if (!res.name.some(name => { return name.display.trim().length })) return;
        ...
        if (misc.hasDupAddressWithin(number.map((num) => { return num.num }), coords)) {
            ...
            let potentialSegs = cluster.break(tmpSegs, nid);
        ...
        let itpFinal = [];
        for (let seg of segs) {
            let itp = interpolize(seg, { debug: opts.debug });
            ...
            if (opts.country) itp.properties['carmen:geocoder_stack'] = opts.country;

            itp = opts.post.feat(itp);
