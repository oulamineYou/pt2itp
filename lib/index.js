...
class Index {
    ...
    init(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE address (id SERIAL, segment BIGINT, text TEXT, text_tokenless TEXT, _text TEXT, number NUMERIC, lon TEXT, lat TEXT, source TEXT, output BOOLEAN, geom GEOMETRY(POINTZ, 4326), netid BIGINT);
                CREATE INDEX ON address (id);

                DROP TABLE IF EXISTS network;
                CREATE TABLE network (id BIGINT, segment BIGINT, text TEXT, text_tokenless TEXT, _text TEXT, named BOOLEAN, geomtext TEXT, geom GEOMETRY(LINESTRINGZ, 4326), network_length NUMERIC);
                ...
                CREATE TABLE address_orphan_cluster(ID SERIAL, text TEXT, _text TEXT, text_tokenless TEXT, geom GEOMETRY(GEOMETRYZ, 4326));
                CREATE TABLE network_cluster(ID SERIAL, text TEXT, _text TEXT, text_tokenless TEXT, geom GEOMETRY(GEOMETRYZ, 4326), source_ids BIGINT[]);
    ...
    /**
     * Import a stream of GeoJSON (Multi)Polygon Features to use to split the clustering operation by
     *
     * @param {string}      path    to itp geojson file
     * @param {Object}      opts    optional args
     * @param {Function}    cb      Callback
                ...
                segQ.awaitAll((err, res) => {
                    ...
                    this.pool.query(`
                        SELECT
                            segment
                        FROM (
                            SELECT segment FROM network GROUP BY segment
                            UNION
