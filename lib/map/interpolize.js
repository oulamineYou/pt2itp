...
function interpolize(splits, argv = {}) {
    ...
    for (const split of splits) {
        const intersections = split.intersections;
        ...
        const distFromLineLimit = distsFromLine[Math.floor(distsFromLine.length / 2)] * 10; // Median * n
...
function itpSort(aFeat, bFeat) {
    const a = aFeat.properties['carmen:lfromhn'] ? aFeat.properties['carmen:lfromhn'] : aFeat.properties['carmen:rfromhn'];
    const b = bFeat.properties['carmen:lfromhn'] ? bFeat.properties['carmen:lfromhn'] : bFeat.properties['carmen:rfromhn'];
