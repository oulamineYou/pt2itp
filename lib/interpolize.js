    ...
    for (let split of splits) {
        if (!split.address || !split.number) {
            if (turf.lineDistance(split.network) < 0.001)
                continue;
            let feat = genFeat(text, split, null, argv);
            if (feat)
                itps.push(feat);
            continue;
        }
        ...
        for (let address_it = 0; address_it < split.address.geometry.coordinates.length; address_it++) {
            ...
            let res = {
                ...
                'geometry': addr,
                'number': parseInt(split.number[address_it].num),
...
function genFeat(text, split, result, argv) {
    ...
    if (split.address && split.number && split.number.once((num) => { return num.output })) {
        ['carmen:parityl', 'carmen:lfromhn', 'carmen:ltohn', 'carmen:parityr', 'carmen:rfromhn', 'carmen:rtohn'].forEach((prop) => {
        ...
        for (let num_it = 0; num_it < split.number.length; num_it++) {
            ...
            res.properties['carmen:addressnumber'][1].push(split.number[num_it].num);
            res.geometry.geometries[1].coordinates.push(split.address.geometry.coordinates[num_it]);
        }
    }
