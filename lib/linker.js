 ...
 * @param {Array} addr Array of Objects containing information about the address
        ...
        for (let net_it = 0; net_it < nets.length; net_it++) {
            ...
            if (addr.tokenless && net.name.tokenless) {
                ...
                for (let atok of addr.tokenized.split(' ')) {
                    if (ntoks.indexOf(atok) > -1) aMatch++;
                }

                if (aMatch / ntoks.length > 0.66) levScore = aMatch / ntoks.length;
