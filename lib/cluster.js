...
class Cluster {
    ...
    network(cb) {
        this.pool.query(`
            ...
            INSERT INTO network_cluster (text, _text, text_tokenless, priority, geom)
                SELECT
                    netw.text,
                    netw._text,
                    netw.text_tokenless,
        ...
        function collapseNetworkClusters(collapseInto, otherClusters, opts, cnCallback) {
            ...
            that.pool.query(`
                ...
                UPDATE network_cluster n1
                    ...
                    FROM (
                        SELECT
                            STRING_AGG(n2._text, ',' ORDER BY n2.priority DESC, n2.id ASC) AS _text,
                            STRING_AGG(n2."text", ',' ORDER BY n2.priority DESC, n2.id ASC) AS "text",
                            STRING_AGG(n2.text_tokenless, ',' ORDER BY n2.priority DESC, n2.id ASC) AS text_tokenless
                        FROM
                            network_cluster n2
