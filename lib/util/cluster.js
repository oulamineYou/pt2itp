...
class Cluster {
    ...
    break(segs, id) {
        ...
        for (let seg_it = 0; seg_it < segs.length; seg_it++) {
            ...
            //Start by finding places where the numbering resets
            //1, 2, 3, 1, 2, 3
            //
            let floating_min = 0;
            let floating_max = 0;
            let floating_delta = null;
            for (let dist_it = 0; dist_it < dist.length; dist_it++) {
                if (!floating_delta) floating_delta = distDelta[dist_it];

                //Anytime the dist delta changes, check it for an address reset
                if (floating_delta !== distDelta[dist_it]) {
                    console.error(dist_it)
                }
