...
class Cluster {
    ...
    break(segs, id) {
        ...
        for (let seg_it = 0; seg_it < segs.length; seg_it++) {
            ...
            if (!breaks.length) orphanSegs.push(seg);

            for (let brk of breaks) {
                const brk_dist = dist[brk].distOnLine + (dist[brk + 1].distOnLine - dist[brk].distOnLine) / 2;

                newSegs.push({
                    network: turf.lineSliceAlong(seg.network, 0, brk_dist)
                });

                seg.network = turf.lineSliceAlong(seg.network, brk_dist, turf.length(seg.network));
