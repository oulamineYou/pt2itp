...
class Cluster {
    ...
    **/
    collapse(callback) {
        ...
        collapseQ.awaitAll((err, results) => { if (err) console.log(err); console.log(`ok - collapsed network clusters in ${Math.round(+(new Date()) - that.startTime)} ms`); process.exit(1); });
        ...
        **/
        function collapseNetworkClusters(collapseInto, otherClusters, cnCallback) {
            console.log(`collapsing ${otherClusters.join(',')} into ${collapseInto}`);
            that.pool.query(`
                BEGIN TRANSACTION;
