...
function modify(known, potential) {
    if (turf.distance(known, potential, 'kilometers') > 0.5) return create(potential);

    let modify = false;
    let names = JSON.parse(JSON.stringify(known.properties.street));

    for (let pname of potential.properties.street) {
        pname = tokenize.replaceToken(tokenRegex, tokenize.main(pname.display, opts.tokens, true).tokens.join(' '));

        let exists = false;
        for (let kname of known.properties.street) {
            kname = tokenize.replaceToken(tokenRegex, tokenize.main(pname.display, opts.tokens, true).tokens.join(' '));
