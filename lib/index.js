    ...
    init(cb) {
        this.pool.connect((err, client, release) => {
            ...
            client.query(`
                ...
                CREATE TABLE address (id SERIAL, segment BIGINT, text TEXT, text_tokenless TEXT, _text TEXT, number NUMERIC, lon TEXT, lat TEXT, source TEXT, output BOOLEAN, geom GEOMETRY(POINTZ, 4326));
            ...
            if (type === 'address') {
                ...
                for (let child of nursery) {
                    addrQ.defer((output, done) => {
                        self.pool.query(`
                            \copy address (text, text_tokenless, _text, lon, lat, number, source, output)
                ...
                for (let child of nursery) {
                    netQ.defer((output, done) => {
                        self.pool.query(`
                            \copy network (id, text, text_tokenless, _text, geomtext, network_length)
                            FROM '${output}' WITH CSV DELIMITER '|' QUOTE E'\b' NULL AS '';
                        `, (err) => {
                            if (err) return done(err);
                            fs.unlink(output, (err) => {
