...
class Compare {
    ...
    constructor(o) {
        ...
        if (!this.opts.read) throw new Error('opts.read path is required'); 
            ...
            while (true) {
                ...
                q.defer((line, done) => {
                    ...
                    try {
                        ...
                        process.stderr.write(`Unable to parse: ${err.toString()}\t${line}\n`);
                        return done(err);
                    }

                    if (feat instanceof Error) {
                        process.stderr.write(`${feat.message}: ${line}\n`);
                        return done();
                    }
                    ...
                    `, (err, res) => {
                        ...
                        if (hecate) self.opts.output.write(JSON.stringify(hecate) + '\n');
