...
function compareAddressAndNetwork(argv, pool, callback) {
    ...
    popQ.defer((done) => {
        pool.query(`
            ...
            SELECT
                ...
                address_frequency,
                frequency_diff,
                (frequency_diff - avg_diff) / stddev_diff as zscore
            FROM
                unigram_side_by_side uc
                    CROSS JOIN
                (SELECT
                    AVG(frequency_diff) AS avg_diff,
                    STDDEV(frequency_diff) as stddev_diff
                 FROM
    ...
    function copyOutput(type, order, outputPath, cb) {
        pool.query(`
            BEGIN;
            COPY significant_${type}_${order}s to '${outputPath}' CSV HEADER;
            COMMIT;`,
            (err, res) => {
                if (err) {
                    pool.end();
