...
class Cluster {
    ...
    **/
    collapse(callback) {
        ...
        **/
        // @TODO: collapse network geometry when IDs aren't identical!
        function collapseNetworkClusters(collapseInto, otherClusters, opts, cnCallback) {
            ...
            if (opts.redoNetworkGeometry) {
                networkGeomRewriteSql = `
                    ...
                    FROM (
                        SELECT
                            ST_Collect(ST_Force2D(MAX(n.geom))) AS geom
                        FROM
                            network n
                        WHERE
                            n.id IN (SELECT UNNEST(nc2.source_ids) FROM network_cluster nc2 WHERE nc2.id=${collapseInto})
                        GROUP BY n.id -- network table id is shared across duplicate geometry w/ diff names
                    ) a
                                ...
                                for (let row of rows) {
                                    ...
                                    for (let child of clusters) {
                                        ...
                                        if (processedIds.indexOf(child.id) === -1) {
                                            let overlapPct = child.overlapLength / parentCluster.network_length;
                                            if (
                                                (overlapPct > 0.95) ||
                                                ((child.address === -1) && (overlapPct > 0.75)) // less stringent criteria for children w/o addresses
                                               ) {
